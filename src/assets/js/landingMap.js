...
const svg = d3.select('#map').append("svg")
  ...
  .attr("width", width)
  .attr("height", height)
  .attr("class", "YlOrRd"); // choice : YlGn, YlGnBu, GnBu, BuGn, PuBuGn, PuBu, BuPu, RdPu, PuRd, OrRd, YlOrRd, YlOrBr, Purples, Blues, Greens, Oranges, Reds, Greys
...
promises.push(d3.csv("data/places.csv"));
...
Promise.all(promises).then(function(values) {
  ...
  var quantile = d3.scaleQuantile()
    .domain([0, d3.max(csv, function(e) { return +e.NB_PLACES; })])
    .range(d3.range(7));
  ...
  var legendScale = d3.scaleLinear()
    .domain([
      d3.min(csv, function(e) { return +e.NB_PLACES; }),
      d3.max(csv, function(e) { return +e.NB_PLACES; })
    ])
    .range([0, 9 * 20]);
  ...
  csv.forEach(function(e,i) {
    d3.select("#d" + e.CODE_DEPT)
      .attr("class", function(d) { return "department q" + quantile(+e.NB_PLACES) + "-9"; })
